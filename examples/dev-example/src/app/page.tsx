...
const EmailBuilderExample = () => {
  ...
  const onSendEmail = async () => {
    if (!emailFields.recipientEmail) {
      toast(
        <p className="text-red-600">Please enter a recipient email address</p>
      );
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        toast(
          <p>
            Email sent successfully! Check your inbox at{" "}
            <b>{emailFields.recipientEmail}</b> for the email you just sent.
          </p>
        );
      } else {
        toast(
      ...
      toast(
        <p className="text-red-600">Error sending email. Please try again.</p>
      );
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="w-full p-10">
      <Toaster />
      <CheckSourceCode directLink="https://github.com/Darginec05/Yoopta-Editor/blob/master/web/next-example/src/components/EmailBuilderExample/EmailBuilderExample.tsx#L291" />
      ...
      <Dialog open={isMailModalOpen} onOpenChange={setIsMailModalOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              We use <b>Resend</b>, so you will need to get the{" "}
              <u>RESEND_API_KEY</u> for the test. You can create token using{" "}
              <a
              ...
              . Then copy the <u>RESEND_API_KEY</u> and paste it in the field
              below.
            </DialogDescription>
          </DialogHeader>
...
export default EmailBuilderExample;
